...
const VolunteerSection = () => {
  ...
  return (
    <section id="VolunteerSection" className="w-full flex flex-col-reverse gap-8 md:flex-row items-center p-8 bg-white">
      <div className="md:w-1/2 flex flex-col p-8 justify-center items-center h-auto md:h-[50rem] text-left">
        <div className="w-full md:w-1/2">
          ...
          {volunteerContent.content.split("\n").map((paragraph, index) => (
            <p
              key={index}
              className="text-gray-600 mb-4 text-xl [font-family:var(--font-sans)]"
            >
              {paragraph}
            </p>
          ))}
          ...
          <button
            onClick={() => {}}
            className="hidden md:block px-6 py-2 border border-black cursor-pointer text-black font-semibold rounded-lg"
          >
            READ MORE
          </button>
        </div>
