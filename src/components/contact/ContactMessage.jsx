...
const ContactMessage = ({
  ...
  // auto-dismiss (except when loading)
  useEffect(() => {
  ...
  const getIcon = () => {
    switch (type) {
      ...
      case 'loading':
        return (
          <div className={`w-6 h-6 ${styles.icon}`} aria-hidden="true">
        ...
        <div className="flex items-start">
          ...
          <div className="ml-3 flex-1">
            <p className={`text-sm font-medium ${styles.text}`}>
              {type === 'success' &&
                t('contact.notifications.successTitle', 'Success!')}
              {type === 'error' &&
                t('contact.notifications.errorTitle', 'Error')}
              {type === 'loading' &&
                t('contact.notifications.loadingTitle', 'Sending...')}
            </p>
            <p className={`mt-1 text-sm ${styles.text} opacity-80`}>{message}</p>
          </div>
          ...
          {type !== 'loading' && (
            <div className="ml-4 flex-shrink-0 flex">
              <button
                ...
                aria-label={t(
                  'common.actions.closeNotification',
                  'Close notification',
                )}
              >
